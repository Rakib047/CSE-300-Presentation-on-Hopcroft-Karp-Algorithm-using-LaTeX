...
\begin{frame}{First iteration:DFS}
    ...
    \only<7>{$\hspace{4.6cm}\text{After }M=M\oplus P$}\\
...
\begin{frame}{Second iteration:DFS}
    ...
    \only<2>{$\hspace{4.6cm}F=\{h\}$}\\
    \only<2>{$\hspace{4.6cm}P=\{\}$}\\
    %\only<2>{$\hspace{4.6cm}M=\{\textcolor{blue}{(e-a)},(f-c),(g-d)\}$}

    \only<3-4>{$\hspace{4.6cm}F=\{\textcolor{red}{h}\}$}\\
    \only<3-4>{$\hspace{4.6cm}P=\{\textcolor{red}{(h-a)}\}$}\\
    %\only<3-4>{$\hspace{4.6cm}M=\{\textcolor{red}{(h-a)},(f-c),(g-d)\}$}

    \only<5>{$\hspace{4.6cm}F=\{\}$}\\
    \only<5>{$\hspace{4.6cm}P=\{(h-a)\}$}\\
    %\only<5>{$\hspace{4.6cm}M=\{(h-a),(f-c),(g-d)\}$}

    \only<6-7>{$\hspace{4.6cm}F=\{\}$}\\
    \only<6-7>{$\hspace{4.6cm}P=\{(h-a),\textcolor{red}{(e-b)}\}$}\\
    %\only<6-7>{$\hspace{4.6cm}M=\{\textcolor{red}{(e-b)},(h-a),(f-c),(g-d)\}$}

    \only<8>{$\hspace{4.6cm}F=\{\}$}\\
    \only<8>{$\hspace{4.6cm}P=\{(h-a),\textcolor{blue}{(a-e)},(e-b)\}$}\\
    %\only<8>{$\hspace{4.6cm}M=\{(e-b),(h-a),(f-c),(g-d)\}$}

    \only<8>{$\hspace{4.6cm}\text{Before } M=\{(a-e),(f-c),(g-d)\}$}\\
    \only<8>{$\hspace{4.6cm}\text{After }M=M\oplus P$}\\
    \only<8>{$\hspace{4.6cm}M=\{(h-a),(e-b),(f-c),(g-d)\}$}\\

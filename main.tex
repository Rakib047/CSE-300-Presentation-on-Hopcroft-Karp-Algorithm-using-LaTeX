...
\draw[edge,color=red,line width=1.5pt] (A) to (E);
\draw[edge,color=red,line width=1.5pt] (A) to (F);
\draw[edge,color=blue,line width=1.5pt] (A) to (H);
\draw[edge,color=red,line width=1.5pt] (B) to (E);
\draw[edge,color=green,line width=1.5pt] (C) to (F);
\draw[edge] (D) to (E);
\draw[edge] (D) to (F);
...
\draw[edge,color=blue,line width=1.5pt] (B) to (E);
\pause
...
As no more free vertex is available,the algorithm terminates and finally we get the following graph with maximum cardinaliy 4\\
        \vspace{.8cm}
\hspace{2.7cm}
    \begin{tikzpicture}[scale=1.5]
\tikzset{vertex/.style = {shape=circle,draw,minimum size=1.5em}}
